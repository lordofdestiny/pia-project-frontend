...
import { ExaminationRequest, Specialization } from "@core/models/specialization";
...
export class SpecializationsComponent implements OnInit {
    ...
    specializations: Specialization[] = this.route.snapshot.data["specializations"];
    ...
    async handleExaminationRequestResponse({
        ...
        this.specializationService.answer_request(examinationId, action).subscribe({
            next: (examination) => {
                ...
                specialization.examinations = [...(specialization.examinations ?? []), examination];
            },
            error: (_error) =>
                this.showResponseDialog({
                    success: false,
                    message: `Failed to ${action ? "accept" : "reject"} examination request!`,
                }),
        });
    }
